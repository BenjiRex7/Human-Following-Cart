...
url = 'http://192.168.100.202/cam-lo.jpg'
...
while(1):
    ...
    if len(classIds) != 0:
        for classId, confidence, box in zip(classIds.flatten(), confs.flatten(), bbox):
            class_index = classId - 1  # Adjust for zero-based indexing
            if class_index >= 0 and class_index < len(classNames):
                cv2.rectangle(img, box, color=(0, 255, 0), thickness=3)
                cv2.putText(img, classNames[class_index], (box[0] + 10, box[1] + 30),
                            cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 2)
            else:
                print(f"Invalid class ID: {classId}")
